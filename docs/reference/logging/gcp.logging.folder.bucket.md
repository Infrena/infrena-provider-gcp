...
| --- | --- | --- |
| Create | **no** | the create url `{+parent}/buckets` needs `folder`, which nothing supplies: no provider setting (project, region, zone, location), no stored binding, and no settable attribute of this type |
| Read | yes | `GET {+name}` |
