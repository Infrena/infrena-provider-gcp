...
| --- | --- | --- |
| Create | **no** | the create url `{{parent}}/locations/{{location}}/logScopes?logScopeId={{logScopeId}}` needs `organization`, which nothing supplies: no provider setting (project, region, zone, location), no stored binding, and no settable attribute of this type |
