...
| --- | --- | --- |
| Create | yes | `POST projects/{{project}}/regions/{{region}}/securityPolicies` |
| Read | yes | `GET projects/{{project}}/regions/{{region}}/securityPolicies/{{name}}` |
| Update | yes | `PATCH projects/{{project}}/regions/{{region}}/securityPolicies/{{name}}`, with an update mask naming the changed fields |
| Delete | yes | `DELETE projects/{{project}}/regions/{{region}}/securityPolicies/{{name}}` |
...
| `advancedOptionsConfig.jsonCustomConfig.contentTypes` | `content_types` | `list` | required | unordered | A list of custom Content-Type header values to apply the JSON parsing. As per RFC 1341, a Content-Type header value has the following format: Content-Type := type "/" subtype *\[";" parameter\] When … |
...
| `advancedOptionsConfig.userIpRequestHeaders` | `user_ip_request_headers` | `list` | optional | unordered | An optional list of case-insensitive request header names to use for resolving the callers client IP address. |
...
| `ddosProtectionConfig.ddosProtection` | `ddos_protection` | `string` | required | — | — |
